...
figure(2);
imshow(Y_b);
title('Blurry single photon Image');
...
t=toc;
fprintf('2D-DWT single photon Deblurring Running Time:%f s\n',t);

figure(3);
imshow(k_estimate,[]);
title('Estimated kernel');
%% Fast single photon analysis Priors non-blind deblurring
I_b=im2double(I);
I_FHLP=I_b;
I_blur=I_b;
tic;
...
t=toc;
fprintf('Wavelet transformation Kernel Run Time: %f s\n',t);
figure(7);
imshow(I_FHLP);
title('Deblur using 2D-DWT');
drawnow
imwrite(I_FHLP,'restored_image_2ddwt.jpg');
